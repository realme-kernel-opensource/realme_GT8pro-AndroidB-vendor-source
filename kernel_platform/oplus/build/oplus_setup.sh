...
function init_build_environment() {
    ...
    STRIP="${ACKDIR}/prebuilts/clang/host/linux-x86/llvm-binutils-stable/llvm-strip"
    KERNEL_OUT=${TOPDIR}/device/qcom/${variants_platform}-kernel
    check_module_tool=${ACKDIR}/oplus/build/check_module_list_tool.py
    INTREE_MODULE_OUT=${ACKDIR}/bazel-bin/msm-kernel/${variants_platform}_${variants_type}
...
function print_module_help ()
{
    ...
    echo "[1] if it is <in tree ko>, you should input directory like this:"
    echo "drivers/power/oplus/v2/oplus_chg_v2.ko"
    echo ""
    echo "[2] if it is <out of tree ko>, you should input directory like this:"
    echo "vendor/oplus/kernel/boot/oplus_phoenix"
    echo "==============================================================================="
...
print_help()
{
    ...
    echo "./kernel_platform/oplus/build/oplus_xxx_xxx.sh sun consolidate"
    echo "user build command"
    echo "./kernel_platform/oplus/build/oplus_xxx_xxx.sh sun perf"
    echo ""
}
print_end_help()
{
